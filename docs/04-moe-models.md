...
```
业界常用公式:
  每个 token 的 FLOPs ≈ 2 × 激活参数量
```

这是整本文档里最重要的一个公式，**后面所有的 FLOPs 数字都是从它推出来的**。下面把它讲透。

##### 推导：为什么是 "2 ×"？

Transformer 里**几乎所有计算量都来自矩阵乘法**（Q/K/V 投影、O 投影、FFN 的 gate/up/down）。我们从最小单元 —— 一次矩阵×向量乘法 —— 开始推导。

**一次矩阵乘法 `y = W · x` 的 FLOPs：**

假设权重矩阵 `W ∈ R^(m×n)`，输入向量 `x ∈ R^n`，输出向量 `y ∈ R^m`：

```
对每个输出元素 y[i]（共 m 个）:
  y[i] = W[i][0]·x[0] + W[i][1]·x[1] + ... + W[i][n-1]·x[n-1]
         └────────── n 次乘法 ──────────┘
         + (n-1) 次加法 ≈ n 次加法

→ 一个 y[i] 需要 2n 次浮点运算  (n 次乘 + n 次加)
→ m 个输出元素总计:  m × 2n = 2 × (m × n) 次浮点运算
                              └───────┘
                              W 的参数数 = m × n
```

**结论：`一次矩阵乘法的 FLOPs = 2 × W 的参数数`**

**推广到整个模型：**

一个 token 过整个 Transformer 时，会依次经过每一层的每一个权重矩阵。每个矩阵贡献 `2 × 自身参数数` 的 FLOPs。把所有矩阵加起来 ≈ 模型总参数数（embedding 和 LayerNorm 占比极小，可忽略），所以：

```
每 token FLOPs ≈ 2 × 参与计算的参数数
               = 2 × 激活参数数
```

对 Dense 模型，"激活参数" = "总参数"；对 MoE 模型，"激活参数" 只包含被选中的专家。

##### 举例计算（Dense 模型）

```
Dense 3B:    2 × 3×10⁹   = 6 GFLOPs
Dense 7B:    2 × 7×10⁹   = 14 GFLOPs
Dense 13B:   2 × 13×10⁹  = 26 GFLOPs
Dense 70B:   2 × 70×10⁹  = 140 GFLOPs
Dense 405B:  2 × 405×10⁹ = 810 GFLOPs
```

##### 举例计算（MoE 模型）：Mixtral 8×7B 为什么是 26 GFLOPs？

这是一个容易绕晕的点 —— Mixtral 总参数 **47B**，但激活参数 **≈13B**，所以每 token FLOPs 是 **26 GFLOPs**（而不是 94 GFLOPs）。

**关键：激活参数 ≠ 总参数 × (Top-K / 专家数)**

很多人会误以为 `激活参数 = 47B × 2/8 = 12B`，这是错的，因为 47B 里**包含非 MoE 部分**（Attention、embedding、共享层），这些部分每次都要全算。

**Mixtral 8×7B 参数拆解：**

| 部分 | 参数量 | 每 token 是否激活 | 贡献 FLOPs |
|---|---|---|---|
| Embedding + LN + 输出头 | ~2 B | ✅ 全部激活 | 2 × 2B = 4 GFLOPs |
| Attention（Q/K/V/O 投影 × 32 层） | ~3 B | ✅ 全部激活 | 2 × 3B = 6 GFLOPs |
| 8 个 MoE FFN 专家（每个 ~5.25B） | 8 × 5.25B = **42 B** | ❌ 只激活 Top-2 ≈ **10.5 B** | 2 × 10.5B = 21 GFLOPs |
| Router | ~32K × 32 层 | ✅ 激活但极小 | ~0 GFLOPs |
| **总参数 / 激活参数** | **47 B** / **≈13 B** | — | **≈26 GFLOPs** |

（注：`5.25B` 是每个专家的近似 FFN 参数数；`21+4+6 ≈ 26 GFLOPs`，四舍五入后一致）

**同样方法算 Dense 47B（假设存在）：**

```
Dense 47B（所有 47B 参数每次都算）:
  每 token FLOPs = 2 × 47B = 94 GFLOPs
```

##### 三组数字的核心意义

| 模型 | 总参数 | 激活参数 | 每 token FLOPs |
|---|---|---|---|
| Dense 7B | 7 B | 7 B | **14 GFLOPs** |
| Mixtral 8×7B（MoE） | **47 B** | **13 B** | **26 GFLOPs** |
| Dense 47B（假想） | 47 B | 47 B | **94 GFLOPs** |

**看明白 MoE 的核心价值了吗：**

- 跟 Dense 7B 比：MoE 总参数大 **6.7 倍**（47B vs 7B，知识容量暴涨），但计算量只大 **2 倍**（26 vs 14）
- 跟 Dense 47B 比：MoE 知识容量相同（都是 47B），但计算量只有 **1/4**（26 vs 94）

这就是 MoE "用**内存**换**计算量**" 的本质。

##### 这个公式的适用范围

| ✅ 适用场景 | ❌ 不适用 / 需额外考虑 |
|---|---|
| Decode 阶段（生成每个新 token） | Prefill 阶段（要 × prompt 长度） |
| 矩阵乘为主的模型（Transformer / LLM） | CNN（卷积算力算法不同） |
| 粗略对比不同模型规模 | 长上下文（要额外加 Attention 的 O(L²) 开销） |
| 工程估算、选型参考 | 精确的性能预测（没考虑内存带宽瓶颈，见下一节） |

严格来说 Attention 还有 `O(seq_len²)` 的计算量（Q·Kᵀ 和 softmax·V），在**长文本场景**才显著。短文本下（几百到几千 token），`2 × 激活参数` 是工程上公认的标准估算公式。
