...
final class LlamaEngine {
    ...
    func generate(
        ...
        while generatedCount < maxTokens && !isCancelled() {
            ...
            // 处理 thinking channel：跳过 <|channel>...<channel|> 之间的内容
            ...
            // 把新 token 喂回去
            let nextBatch = llama_batch_get_one(UnsafeMutablePointer(mutating: [newToken]), 1)
    ...
    private func applyChatTemplate(messages: [(role: String, content: String)]) -> String {
        ...
        // 2. 直接使用 Gemma 4 格式手动构造 prompt
        //    Gemma 4 turn 标记：开始 <|turn>  结束 <turn|>
        //    格式: <|turn>user\ncontent<turn|>\n<|turn>model\n
        //    注意：不要用 "gemma" 预定义模板，那是 Gemma 1/2 格式（<start_of_turn>）
